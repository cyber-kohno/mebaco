...
<section class="client-home-screen" aria-label={$translatorStore('client.packages.screen.label')}>
  <header class="screen-header">
    <div>
      <h1>{$translatorStore('client.packages.screen.title')}</h1>
      <p>{$translatorStore('client.packages.screen.description')}</p>
    </div>
    <button class="install-button" type="button" disabled={installing} onclick={() => fileInput.click()}>
      <PackagePlus size={17} strokeWidth={2.2} />
      {$translatorStore(installing ? 'client.packages.action.installing' : 'client.packages.action.install')}
    </button>
  ...
  <div class="split-pane">
    <aside class="package-pane" aria-label={$translatorStore('client.packages.list.label')}>
      <div class="pane-heading">
        <span>{$translatorStore('client.packages.list.title')}</span>
      ...
      {#if $packageStore.packages.length === 0}
        <div class="list-empty">
          ...
          <strong>{$translatorStore('client.packages.empty.title')}</strong>
          <span>{$translatorStore('client.packages.empty.description')}</span>
        </div>
      {:else}
          ...
          {#each $packageStore.packages as item (item.installationId)}
            ...
            >
              ...
              <span class="package-copy">
                ...
                <span>{$translatorStore(
                  item.manifest.bundle.launcherCount === 1
                    ? 'client.packages.item.launchers.one'
                    : 'client.packages.item.launchers.many',
                  { count: item.manifest.bundle.launcherCount },
                )}</span>
              </span>
              <span class="status-dot" title={$translatorStore('client.packages.status.compatible')}></span>
            </button>
          {/each}
        </div>
      {/if}
    ...
    <main class="detail-pane">
      {#if selectedPackage == null}
        <div class="detail-empty">
          ...
          <strong>{$translatorStore('client.packages.detail.empty.title')}</strong>
          <span>{$translatorStore('client.packages.detail.empty.description')}</span>
        </div>
      {:else}
        <header class="detail-header">
          <div class="detail-title">
            {#if renamingId === selectedPackage.installationId}
              <label for="package-display-name">{$translatorStore('client.packages.detail.displayName')}</label>
              <div class="rename-row">
                ...
                />
                <button type="button" class="compact primary" disabled={renameValue.trim().length === 0} onclick={saveRename}>{$translatorStore('common.action.save')}</button>
                <button type="button" class="compact" onclick={() => renamingId = null}>{$translatorStore('common.action.cancel')}</button>
              </div>
            {:else}
              <span class="eyebrow">{$translatorStore('client.packages.detail.applicationPackage')}</span>
          ...
          {#if renamingId !== selectedPackage.installationId}
            <div class="detail-actions">
              <button type="button" onclick={startRename}><Pencil size={15} />{$translatorStore('client.packages.action.rename')}</button>
              <button type="button" disabled={updating} onclick={selectUpdateFile}>
                <RefreshCw size={15} />{$translatorStore(updating ? 'client.packages.action.updating' : 'client.packages.action.update')}
              </button>
              <button class="danger" type="button" onclick={() => ClientPackageController.deletePackage(selectedPackage.installationId)}><Trash2 size={15} />{$translatorStore('common.action.delete')}</button>
              <button class="primary" type="button" onclick={() => ClientNavigation.openLaunchSetup(selectedPackage.installationId)}>
                {$translatorStore('client.packages.detail.launchSetup')}<ArrowRight size={16} />
              </button>
            </div>
          {/if}
        </header>
        ...
        <div class="detail-content">
          <section class="status-card">
            ...
            <div>
              <strong>{$translatorStore('client.packages.status.title')}</strong>
              <span>{selectedPackage.manifest.bundle.resourceCount === 0
                ? $translatorStore('client.packages.status.description.none')
                : $translatorStore(
                    selectedPackage.manifest.bundle.resourceCount === 1
                      ? 'client.packages.status.description.one'
                      : 'client.packages.status.description.many',
                    { count: selectedPackage.manifest.bundle.resourceCount },
                  )}</span>
            </div>
          </section>
          ...
          <div class="information-grid">
            <section class="information-card">
              <h3>{$translatorStore('client.packages.detail.information.package')}</h3>
              <dl>
                ...
                <div><dt>{$translatorStore('client.packages.detail.information.built')}</dt><dd>{formatDate(selectedPackage.manifest.bundle.builtAt)}</dd></div>
                <div><dt>{$translatorStore('client.packages.detail.information.created')}</dt><dd>{formatDate(selectedPackage.manifest.createdAt)}</dd></div>
                <div><dt>{$translatorStore('client.packages.detail.information.lastUpdated')}</dt><dd>{formatDate(selectedPackage.updatedAt)}</dd></div>
                <div><dt>{$translatorStore('client.packages.detail.information.fileSize')}</dt><dd>{formatBytes(selectedPackage.byteLength)}</dd></div>
                ...
                <div><dt>{$translatorStore('client.packages.detail.information.createdWith')}</dt><dd>Mebaco {selectedPackage.manifest.appVersion}</dd></div>
              </dl>
            </section>

            <section class="information-card contents-card">
              <h3>{$translatorStore('client.packages.detail.information.contents')}</h3>
              <div class="metrics">
              ...
              <div class="launcher-summary">
                <span>{$translatorStore('client.packages.detail.includedLaunchers')}</span>
