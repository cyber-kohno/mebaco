...
import StyleValueSupport from '@system/model/view/style/style-value-support'
...
import type StyleReferencePreview from '@system/runtime/style/style-reference-preview'
import type HtmlTag from '@system/model/element/html-tag'
...
namespace ElementEditSchema {
  ...
  export type CodeField = {
    ...
    getAllowAwait: (values: Readonly<Record<string, string>>) => boolean
  } & FieldBase
  ...
  export const validateText = (
    ...
    if (
      field.charset === 'strictKebabIdentifier'
      && value.length > 0
      && !AppId.isValid(value)
    ) {
      return 'Use lowercase kebab-case. Start each segment with a letter.'
    }
  ...
  export const validateStyleBases = (field: StyleBasesField, value: string): string | null => {
    try {
      ...
      const hasInvalid = parsed.some((item) => {
        ...
        const typedArguments = argumentsValue.filter((item): item is StyleElement.Argument => (
          item != null && typeof item === 'object'
        ))
        if (
          typedArguments.length !== argumentsValue.length
          || StyleArgumentContract.getInvariantError(
            typedArguments,
  ...
  export const validateSwitchValueType = (
    ...
    if (primitive !== field.caseValueType) {
      return `Switch value type must remain ${field.caseValueType} while cases exist.`
    }

    const allowedValues = definition.type === 'primitive'
      ? definition.literals ?? null
      : field.literalUnionOptions.find((option) => (
          option.value === definition.unionTypeId
        ))?.values ?? null
    if (allowedValues == null) return null

    const invalidValues = (field.caseValues ?? []).filter((caseValue) => (
      !allowedValues.some((allowedValue) => Object.is(allowedValue, caseValue))
    ))
    if (invalidValues.length === 0) return null

    const values = invalidValues.map((value) => JSON.stringify(value)).join(', ')
