...
namespace ElementSearchQuery {
  ...
  export const filter = (
    ...
    if (mode === 'node-id') {
      const nodeIdSource = source.trim().replace(/^node-/i, '')
      if (nodeIdSource.length === 0) return entries.slice()
      if (!/^\d+$/.test(nodeIdSource)) return []

      const nodeId = Number(nodeIdSource)
