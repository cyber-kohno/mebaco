<script lang="ts">
  ...
  const isNodeIdSearch = $derived(session?.mode === 'node-id')
  const results = $derived(
    session == null
      ? []
      : ElementSearchQuery.filter(session.entries, session.query, session.mode),
  )
...
{#if session != null}
  <div class="layer" use:bodyPortal>
    ...
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={isNodeIdSearch ? 'Search nodes by node ID' : 'Search elements'}
      tabindex="-1"
    ...
    >
      <div class="search-input-row">
        <input
          ...
          value={session.query}
          aria-label={isNodeIdSearch
            ? 'Search nodes by node ID'
            : 'Search elements by ID and kind'}
          ...
          autocomplete="off"
          spellcheck="false"
          inputmode={isNodeIdSearch ? 'numeric' : 'text'}
          placeholder={isNodeIdSearch ? 'Node ID' : 'ID kind'}
          oninput={handleInput}
        />
      </div>
      ...
      <div
        ...
        role="listbox"
        aria-label={isNodeIdSearch ? 'Node search results' : 'Element search results'}
        onscroll={(event) => {
      ...
      >
        {#if results.length === 0}
          <div class="empty-result">
            {isNodeIdSearch ? 'No nodes found.' : 'No elements found.'}
          </div>
        {:else}
          <div
            class="result-space"
              ...
              >
                ...
                <span class="id-column">
                  <HighlightedSearchText
                    ...
                    searchText={isNodeIdSearch
                      ? session.query.trim()
                      : query.id.text}
                    exact={isNodeIdSearch || query.id.exact}
                  />
                </span>
              </button>
            {/each}
