...
describe('ElementSearchQuery', () => {
  ...
  it('finds an exact numeric node ID with or without the node prefix', () => {
    const entries = [
      entry('apps', 'node-2', 2),
      entry('tag', 'node-22', 22),
      entry('text', 'node-220', 220),
    ]

    expect(ElementSearchQuery.filter(entries, '22', 'node-id')).toEqual([
      entries[1],
    ])
    expect(ElementSearchQuery.filter(entries, 'node-22', 'node-id')).toEqual([
      entries[1],
    ])
    expect(ElementSearchQuery.filter(entries, '', 'node-id')).toEqual(entries)
    expect(ElementSearchQuery.filter(entries, '22 tag', 'node-id')).toEqual([])
  })
})
