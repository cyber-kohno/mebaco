...
namespace DirectoryResourceElementDefinition {
  export const createSchema = (
    options: { reservedNames?: readonly string[] } = {},
  ): ElementEditSchema.Schema<DirectoryResource.Element> => ({
    ...
    createPreview: () => DirectoryResource.create('...', 'preview'),
    ...
    create: (values) => fromValues(DirectoryResource.create(values.id), values),
    update: (element, values) => fromValues({ ...element, id: values.id }, values),
  })

  const fromValues = (element: DirectoryResource.Element, values: Record<string, string>): DirectoryResource.Element => ResourceDefinition.withOptionalName({
    ...element,
    permissions: {
  ...
  } satisfies ElementDefinition.Definition<DirectoryResource.Element>
}

export default DirectoryResourceElementDefinition
