...
import ValuePropElementDefinition from '@system/workspace/element-definition/component/value-prop-element-definition'
import StateElement from '@system/workspace/element-definition/variable/store/state-element-definition'
...
describe.each([
  ...
  ['Value Prop', ValuePropElementDefinition.createSchema()],
...
describe('shared typed default value schema', () => {
  ...
  it('keeps Launch Argument and Value Prop field definitions identical', () => {
    const launchFields = JSON.parse(JSON.stringify(LaunchArgumentElementDefinition.createSchema().fields))
    const propFields = JSON.parse(JSON.stringify(ValuePropElementDefinition.createSchema().fields))

    expect(launchFields).toEqual(propFields)
  })

  it('applies the shared value conversion when creating both elements', () => {
    expect(LaunchArgumentElementDefinition.createSchema().create(values)).toEqual({
    ...
    expect(ValuePropElementDefinition.createSchema().create(values)).toMatchObject({
      kind: 'value-prop',
      id: 'scores',
      valueType: { type: 'array', item: { type: 'number' } },
      nullable: false,
  ...
  it('preserves the Value Prop identity when applying shared updates', () => {
    ...
    expect(ValuePropElementDefinition.createSchema().update(element, values)).toMatchObject({
      kind: 'value-prop',
  ...
  it('provides a friendly Type default preview through the shared schema', () => {
    const schema = LaunchArgumentElementDefinition.createSchema({
