...
import StyleTreeLabel from '@system/workspace/tree/label/view/style/StyleTreeLabel.svelte'
...
import StyleLocalsElementDefinition from './style-locals-element-definition'

namespace StyleElementDefinition {
  type Element = Style.Element
  type Base = Style.Base
  type Argument = Style.Argument
  type ArgumentBinding = Style.ArgumentBinding
  type ParameterValue = Style.ParameterValue
  type FormulaSource = Style.FormulaSource
  type Rule = Style.Rule
  type State = Style.State
  type StateRule = Style.StateRule
  type DeclarationRule = Style.DeclarationRule
  type StyleValue = Style.StyleValue
  type AnimationItem = Style.AnimationItem
  type AnimationRule = Style.AnimationRule
  const states = Style.states
  const create = Style.create
  const getContainerInsertIndex = Style.getContainerInsertIndex
  ...
  export const definition = {
    ...
    getContextMenu: (context) => {
      ...
      const localsNode = context.node.children.find((node) => (
        node.element.kind === 'style-locals'
      ))
      const parameterAction = paramsNode == null
        ? action('Use parameters', () => {
            TreeStore.addChild(
              context.node.id,
              StyleParams.create(),
              getContainerInsertIndex(context.node, 'style-params'),
            )
          })
      ...
      const localsAction = localsNode == null
        ...
        : action('Remove locals', () => {
            StyleLocalsElementDefinition.requestDelete(context.rootNode, localsNode)
          })
...
export default StyleElementDefinition
