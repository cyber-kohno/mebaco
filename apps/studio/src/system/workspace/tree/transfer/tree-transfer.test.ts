...
vi.mock('@system/application/validation/expression', () => ({
  ExpressionVerificationRunner: { verify: vi.fn() },
}))
import Block from '@system/model/block/block'
import RetentionElement from '@system/model/component/retention'
import StylesElement from '@system/model/declaration/styles'
import TypesElement from '@system/model/declaration/types'
import FunctionProcedureElement from '@system/model/function/function-procedure'
import ProjectElement from '@system/model/project/project'
...
import ConditionalElement from '@system/model/directive/conditional'
import IfElement from '@system/model/directive/if'
import ElseElement from '@system/model/directive/else'
...
describe('TreeTransfer', () => {
  ...
  it('exposes only structurally compatible paste destinations', () => {
    ...
    const object = node(5, ObjectType.create('User', 'type-id'))
    const retentionBlock = node(8, Block.create())
  ...
  it('offers only View content destinations for a Tag', () => {
    ...
    const viewBlock = node(9, Block.create())
    ...
    const retentionBlock = node(14, Block.create())
    const retention = node(13, RetentionElement.create(), [retentionBlock])
    const procedureBlock = node(16, Block.create())
  ...
  it('offers the same View content destinations for a Loop as a Tag', () => {
    ...
    const retentionBlock = node(11, Block.create())
  ...
  it('moves an Object Type while preserving its Type and Property identities', async () => {
    ...
    const source = node(5, ObjectType.create('Payload', 'payload-type', [
  ...
  it('rejects an Object Type Move when its Id already exists at the destination', () => {
    const source = node(5, ObjectType.create('Payload', 'source-type'))
    const existing = node(9, ObjectType.create('Payload', 'existing-type'))
    const sourceTypes = node(4, TypesElement.create(), [source])
  ...
  it('rejects an Object Type Move that would break an incoming Type reference', () => {
    const source = node(5, ObjectType.create('Payload', 'payload-type'))
    const consumer = node(6, ObjectType.create('Envelope', 'envelope-type', [
      TypeExpression.createProperty(
        'payload',
        TypeExpression.createReference(['payload-type']),
        'payload-property',
  ...
  it('rejects a Union Type Move when a referenced Object is unavailable there', () => {
    const object = node(5, ObjectType.create('Payload', 'payload-type'))
    const source = node(6, {
      kind: 'union-type',
      id: 'Result',
      typeId: 'result-type',
  ...
  it('rejects a Union Type Move that would break an incoming named Type reference', () => {
    ...
    const consumer = node(6, ObjectType.create('Task', 'task-type', [
      TypeExpression.createProperty(
        'status',
  ...
  it('rejects a Signature Type Move when a Parameter Type is unavailable there', () => {
    const payload = node(5, ObjectType.create('Payload', 'payload-type'))
    const source = node(6, {
      kind: 'signature-type',
      id: 'Handler',
      typeId: 'handler-signature',
  ...
  it('moves a Procedure Function with its local definitions and identities intact', () => {
    const localObject = node(7, ObjectType.create(
      'Payload',
      'local-object',
      [TypeExpression.createProperty(
        'value',
  ...
  it('copies an unnamed Loop and refreshes owned identities in its subtree', () => {
    ...
    const localType = node(5, ObjectType.create(
      'LoopItem',
      'loop-item-type',
      [localProperty],
    ))
  ...
  it('moves a Loop while preserving its node and owned identities', () => {
    const localType = node(5, ObjectType.create('LoopItem', 'loop-item-type'))
    const source = node(3, LoopElement.createCount('4', 'index'), [
      node(4, { kind: 'retention' }, [localType]),
      node(6, { kind: 'elements' }),
    ])
  ...
  it('copies a Conditional subtree and refreshes owned identities', () => {
    const localType = node(6, ObjectType.create('BranchData', 'branch-data-type'))
  ...
  it('remaps recursive Object references and nested Property identities', () => {
    ...
    const source = node(3, ObjectType.create(
      'Node',
      'source-type',
      [recursive, inline],
    ))
  ...
  it('rejects duplicate destination names before changing the tree', () => {
    const source = node(3, ObjectType.create('User', 'source-type'))
    const existing = node(4, ObjectType.create('UserCopy', 'existing-type'))
    ...
    const root = node(1, ProjectElement.create(), [
      node(2, { kind: 'common' }, [
        node(3, { kind: 'declares' }, [
          node(10, { kind: 'types' }, [
            node(11, ObjectType.create('External', 'external-object')),
          ]),
          functions,
        ]),
      ]),
