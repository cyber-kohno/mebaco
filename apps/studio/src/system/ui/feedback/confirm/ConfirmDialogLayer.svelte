...
{#if $confirmDialogStore != null}
  <div class="overlay" role="presentation" onclick={(event) => { if (event.target === event.currentTarget) ConfirmDialogController.clear() }} onkeydown={(event) => event.stopPropagation()}>
    <section class="dialog" data-tone={$confirmDialogStore.tone} role="alertdialog" aria-modal="true" aria-label={$confirmDialogStore.title ?? $translatorStore('common.dialog.confirmation')}>
...
<style>
  ...
  .dialog[data-tone='warning'] { border-color: #d58b4a; }
