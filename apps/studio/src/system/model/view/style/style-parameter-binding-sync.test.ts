...
import StyleParamElement from './style-param'
...
describe('Style Parameter binding synchronization', () => {
  ...
  it('resets all expression verification when a parameter is added', () => {
    ...
    const currentBase = getNode(2)
    ExpressionVerificationStore.setResult(currentBase, {
      status: 'error',
      messages: ['accent is not defined.'],
    })
