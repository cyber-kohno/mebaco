...
import type FunctionDefinition from '@system/model/function/function-definition'
...
namespace SignatureParameterRefactor {
  ...
  export const apply = (
    ...
    previousElement: SignatureType.Element,
    currentElement: SignatureType.Element,
  ): Result => {
  ...
  export type FunctionResult = Result & {
    element: FunctionDefinition.Element
  }

  export const applyFunction = (
    rootNode: TreeNode.Node,
    functionNodeId: number,
    previousElement: FunctionDefinition.Element,
    currentElement: FunctionDefinition.Element,
  ): FunctionResult => {
    if (
    ...
    return {
      rootNode: resultRoot,
      element: rewrittenElement.value as FunctionDefinition.Element,
      changedNodeIds,
