...
import SequentialVariableScope from '@system/model/variable/sequential-variable-scope'
...
namespace MebacoInjectionSource {
  ...
  ): State.Element[] => targetNode == null
    ? []
    : StateScope.collectVisible(rootNode, targetNode.id)
        .map((entry) => entry.element)

  const getValueType = (
    state: Pick<State.Element, 'valueType' | 'nullable'>,
    rootNode: TreeNode.Node,
