<script lang="ts">
  ...
  const selectedParameter = $derived(
    selected === 'root' || selected === 'return'
      ? null
      : definition.parameters.find((parameter) => parameter.parameterId === selected) ?? null,
  )
  const selectedParameterIndex = $derived(
    selected === 'root' || selected === 'return'
      ? -1
      : definition.parameters.findIndex((parameter) => parameter.parameterId === selected),
  )
  ...
  const updateSelectedParameter = (
    ...
    const next = $state.snapshot(definition)
    const parameter = selected === 'root' || selected === 'return'
      ? null
      : next.parameters.find((candidate) => candidate.parameterId === selected)
  ...
  const deleteSelectedParameter = () => {
    const next = $state.snapshot(definition)
    if (selected === 'root' || selected === 'return') return
    const index = next.parameters.findIndex((parameter) => parameter.parameterId === selected)
    if (index < 0) return
    next.parameters.splice(index, 1)
    emit(next)
    selected = 'root'
  }

  const moveSelectedParameter = (offset: -1 | 1) => {
    if (selectedParameterIndex < 0) return
    const nextIndex = selectedParameterIndex + offset
    if (nextIndex < 0 || nextIndex >= definition.parameters.length) return
    const next = $state.snapshot(definition)
    const current = next.parameters[selectedParameterIndex]
    next.parameters[selectedParameterIndex] = next.parameters[nextIndex]
    next.parameters[nextIndex] = current
    emit(next)
  }
...
<div class="signature-editor">
  ...
  <div class="split-pane">
    <section class="tree-pane" aria-label="Signature structure">
      ...
      {#each definition.parameters as parameter (parameter.parameterId)}
        <button
          type="button"
          class:active={selected === parameter.parameterId}
      ...
      <footer class="operation-footer">
        ...
        {:else if selectedParameter != null}
          ...
          <button
            ...
            disabled={selectedParameterIndex < 0 || selectedParameterIndex >= definition.parameters.length - 1}
            onclick={() => moveSelectedParameter(1)}
          >
            <ArrowDown size={16} />
          </button>
