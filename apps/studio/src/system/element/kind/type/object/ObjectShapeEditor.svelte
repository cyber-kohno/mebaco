<script lang="ts">
  ...
  const rows = $derived.by(() => {
    ...
    const collect = (
      ...
      level.forEach((property) => {
        const path = [...parentPath, property.propertyId]
  ...
  const getLevel = (
    ...
    let level = root
    for (const propertyId of parentPath) {
      const property = level.find((candidate) => candidate.propertyId === propertyId)
  ...
  const getSelectedProperty = (): TypeExpression.Property | null => {
    ...
    return level?.find((property) => property.propertyId === selectedPath.at(-1)) ?? null
  }
  ...
  const validationIssue = $derived.by(() => (
    errorMessage == null
      ? null
      : ObjectShape.inspect(
          ObjectShape.create(properties, baseObjectIds),
          objectOptions,
          namedTypeOptions,
        )
  ))

  const isPropertyInvalid = (propertyId: string): boolean => (
    validationIssue?.type === 'local-property-conflict'
    && validationIssue.propertyId === propertyId
  )

  const isBaseInvalid = (baseObjectId: string): boolean => (
    validationIssue != null
    && validationIssue.type !== 'shape'
    && validationIssue.baseObjectIds.includes(baseObjectId)
  )

  const selectedPropertyError = $derived(
    selectedProperty != null && isPropertyInvalid(selectedProperty.propertyId)
      ? validationIssue?.message ?? null
      : null,
  )

  const baseObjectsError = $derived(
    validationIssue != null
    && validationIssue.type !== 'shape'
    && validationIssue.baseObjectIds.length > 0
  ...
  const updateSelected = (
    ...
    const property = level?.find((candidate) => candidate.propertyId === selectedPath.at(-1))
  ...
  const deleteSelected = () => {
    ...
    if (level == null) return
    const index = level.findIndex((property) => property.propertyId === selectedPath.at(-1))
    if (index < 0) return
  ...
  <div class="split-pane">
    ...
    <section class="detail-pane" aria-label="Selected property">
      <div class="detail-scroll">
          ...
          <div class="type-detail-field root-base-field">
            ...
            <div class="reference-list">
              {#each baseObjectIds as baseObjectId, index}
                <div class:validation-error={isBaseInvalid(baseObjectId)} class="reference-row">
                  <select
                    value={baseObjectId}
                    aria-label={`Base Object ${index + 1}`}
                    aria-invalid={isBaseInvalid(baseObjectId)}
              ...
              {#if baseObjectsError != null}
                <span class="detail-error base-error">{baseObjectsError}</span>
              {/if}
            </div>
          </div>
        {:else}
        ...
        <label class:validation-error={selectedPropertyError != null}>
          <span>Name</span>
          <input
            type="text"
            value={selectedProperty.id}
            aria-invalid={selectedPropertyError != null}
            oninput={(event) => {
              updateSelected((property) => {
          ...
          />
          {#if selectedPropertyError != null}
            <span class="detail-error">{selectedPropertyError}</span>
          {/if}
        </label>
...
<style>
  ...
  .tree-row.validation-error {
    box-shadow: inset 3px 0 #c94758;
    background: #fbe8eb;
  }

  .tree-row.validation-error .property-name {
    color: #b52d41;
  }
  ...
  label.validation-error input,
  .reference-row.validation-error select {
    border-color: #c94758;
    box-shadow: 0 0 0 1px rgba(201, 71, 88, 0.2);
  }
