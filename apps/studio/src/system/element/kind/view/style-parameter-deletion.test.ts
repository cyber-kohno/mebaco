...
describe('Style Parameter deletion', () => {
  ...
  it('checks only expressions that survive UUID argument cleanup', () => {
    ...
    const derivedStyle = node(5, StyleElement.create('derived', [], [{
      ...
      arguments: [
        ...
        {
          parameterId: 'other-parameter',
          binding: {
            type: 'value',
            value: { type: 'formula', source: '$param.accent + "survives"' },
          },
        },
      ],
    }], 'derived-style'))
    const root = node(1, { kind: 'project' }, [baseStyle, derivedStyle])
    ...
    expect(ReferenceGraph.build(request.rootNode, parameter.id).references
      ...
      .toEqual([baseStyle.id, derivedStyle.id])
