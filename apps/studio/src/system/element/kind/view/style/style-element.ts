...
namespace StyleElement {
  ...
  export const getContainerInsertIndex = (
    node: TreeNode.Node,
    kind: StyleParamsElement.Kind | StyleLocalsElement.Kind,
  ): number => {
    if (kind === 'style-params') return 0
    const parametersIndex = node.children.findIndex((child) => (
      child.element.kind === 'style-params'
    ))
