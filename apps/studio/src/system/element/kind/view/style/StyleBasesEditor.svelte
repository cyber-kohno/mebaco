<script lang="ts">
  ...
  const getArgument = (
    ...
    if (argument == null) {
      throw new StyleArgumentContract.InvariantError(
        `Style parameter '${parameter.id}' has no argument.`,
      )
    }
    return argument
  }
  ...
  const createArguments = (
    ...
    const resolution = getResolution(styleId)
    if (resolution.issues.length > 0) return []
    return StyleArgumentContract.createArguments(resolution.parameters, usage)
  }
