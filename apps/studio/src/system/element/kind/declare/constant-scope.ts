...
import type ConstantElement from './constant-element'

namespace ConstantScope {
  export type Entry = {
    node: TreeNode.Node & { element: ConstantElement.Element }
    element: ConstantElement.Element
  }
  ...
  const getEntries = (owner: TreeNode.Node | undefined): Entry[] => {
    ...
    return manager?.children
      .filter((node): node is TreeNode.Node & { element: ConstantElement.Element } => (
